...
public class MenuAdapter extends RecyclerView.Adapter<MenuAdapter.FoodViewHolder> {
    ...
    public MenuAdapter(ArrayList<Coffee> coffeeList, FoodListener foodListener) {
    ...
    public void onBindViewHolder(@NonNull FoodViewHolder holder, int position) {
        holder.txtName.setText(coffeeList.get(position).getName());
        Picasso.with(foodListener.getContext()).load(coffeeList.get(position).getImage()).into(holder.imgFood);
        String status = coffeeList.get(position).getStatus();
